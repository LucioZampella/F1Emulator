...
## schedule = schedule[schedule["RoundNumber"] <= 6]
sessions = schedule_query.get_all_qualifying_sessions(schedule)
teams = sessions[0].results["TeamName"].unique()
...
for session in sessions:
    ...
    for team in teams:
        try:
            ...
            lines.append(f"Round {session.event.RoundNumber:2} — ERROR: {ex}")
            continue

        if result is None:
            lines.append(f"Round {session.event.RoundNumber:2} — None")
            continue
...
for team in teams:
    ...
    if diff is None:
        ...
        lines.append(
            f"\n{team} gap: {diff.avg_delta_pct:.3f}% "
