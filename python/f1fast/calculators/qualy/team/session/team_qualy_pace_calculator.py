...
from python.f1fast.domain.team_pace_result import TeamQualyPace


def get_all_team_paces(paces: list[DriverQualyPace]) -> list[TeamQualyPace]:
    team_paces = dict()
    team_drivers = dict()
    for driver_pace in paces:
        if driver_pace.team not in team_paces:
            team_paces[driver_pace.team] = (driver_pace.qualy_time / 2)
...
def get_pace_for_a_team(paces: list[DriverQualyPace], team: str) -> TeamQualyPace:
...
def get_reference(paces: list[DriverQualyPace]) -> float:

    teams_pace = dict()
    for driver_pace in paces:
        if driver_pace.team not in teams_pace:
            teams_pace[driver_pace.team] = (driver_pace.qualy_time / 2)
        else:
            teams_pace[driver_pace.team] += (driver_pace.qualy_time / 2)
